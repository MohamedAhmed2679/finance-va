...
export default function SocialShareMenu() {
    ...
    const shareText = encodeURIComponent('Check out Finance VA – the smartest way to track your finances!');

    function getShareLink(name: string) {
        switch (name) {
            case 'WhatsApp': return `https://wa.me/?text=${shareText}%20${shareUrl}`;
            case 'Twitter': return `https://twitter.com/intent/tweet?text=${shareText}&url=${shareUrl}`;
            case 'Facebook': return `https://www.facebook.com/sharer/sharer.php?u=${shareUrl}`;
            case 'LinkedIn': return `https://www.linkedin.com/sharing/share-offsite/?url=${shareUrl}`;
            case 'Instagram': return `https://www.instagram.com/`;
            case 'Reddit': return `https://www.reddit.com/submit?url=${shareUrl}&title=${shareText}`;
            default: return '#';
        }
    }

    return (
        <div className={`social-share-menu ${open ? 'active' : ''}`}>
            {SOCIALS.map((s, i) => (
                <a
                    key={s.name}
                    href={getShareLink(s.name)}
                    target="_blank"
                    ...
                    style={{
                        '--i': i,
                        '--clr': s.color,
                        transitionDelay: open ? `${i * 60}ms` : `${(SOCIALS.length - i) * 30}ms`,
